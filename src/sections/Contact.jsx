    ...
    <>
      ...
      >
        ...
        <div className='relative z-10 w-full flex flex-col md:flex-row items-center gap-10'>
          ...
          <motion.div
            className='w-full md:1/2 bg-white/5 p-8 rounded-2xl shadow-lg border  border-white/10'
            initial={{ opacity: 0, x: 50 }}
            whileInView={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.6 }}
          >
            ...
            <form className='flex flex-col gap-5' onSubmit={handleSubmit}>
              ...
              {status && (
                <p className={`text-sm ${status === 'success' ? "text-green-400" : status === "error" ? "text-red-400" : "text-yellow-400"}`}>
                  {status === "sending" ? "sending..." : status === "success" ? "Message Sent Successfuly ✅" : "Somthing Went Wrong ❌"}
                </p>
              )}
